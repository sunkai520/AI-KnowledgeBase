...
async function loadConfig() {
  try {
    ...
    if (res.chat)      Object.assign(form.chat,      { provider: res.chat.provider || "deepseek",  modelName: res.chat.modelName || "",      temperature: res.chat.temperature ?? 0.7,      streaming: res.chat.streaming ?? true, contextWindow: res.chat.contextWindow ?? 32000 });
...
async function handleSave() {
  ...
  try {
    const payload = JSON.parse(JSON.stringify({
      ...
      chat:             { provider: form.chat.provider,      modelName: form.chat.modelName,      temperature: form.chat.temperature,      streaming: form.chat.streaming, contextWindow: form.chat.contextWindow },
